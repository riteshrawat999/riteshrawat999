<h1 align="center">Hi 👋, I'm Ritesh Kumar</h1>
<h3 align="center">🚀 Flutter Developer | Mobile App Enthusiast</h3>

---

## 🧑‍💻 About Me
- 🎯 Aspiring **Flutter Developer** focused on building real-world apps  
- 📱 Skilled in creating **scalable & production-ready mobile applications**  
- 🔥 Strong interest in ** System Design, and Backend Development**  
- 💡 Love solving real-world problems through code  
- 🚀 Currently improving **Backend (Node.js + PostgreSQL)**  

---

## ⚡ Current Focus
- 🔭 Working on: **Production-level Flutter Apps**
- 🌱 Learning: **Clean Architecture, Riverpod, System Design**
- 🤝 Need help with: **Advanced Backend + Scaling Apps**
- 💬 Ask me about: **Flutter, Firebase, API Integration, State Management**
- ⚡ Fun fact: I can spend hours debugging and still enjoy it 😄  

---

## 🌐 Connect With Me
<p align="left">
<a href="https://www.linkedin.com/in/ritesh-flutter/" target="blank">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>
<a href="https://medium.com/@fluttergyaan" target="blank">
<img src="https://img.shields.io/badge/Medium-12100E?style=for-the-badge&logo=medium&logoColor=white" />
</a>
<a href="mailto:ritesh.flutter@gmail.com">
<img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
</a>
</p>

---

## 🛠️ Tech Stack

### 🚀 Mobile Development
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white)
![Dart](https://img.shields.io/badge/Dart-0175C2?style=for-the-badge&logo=dart&logoColor=white)

### 🧠 State Management
- Bloc | Provider | GetX | Riverpod (Learning)

### 🔥 Backend & Database
![NodeJS](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-039BE5?style=for-the-badge&logo=firebase)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-07405E?style=for-the-badge&logo=sqlite&logoColor=white)

### ☁️ Cloud & Tools
![Google Cloud](https://img.shields.io/badge/GoogleCloud-4285F4?style=for-the-badge&logo=google-cloud&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-121011?style=for-the-badge&logo=github)
![Git](https://img.shields.io/badge/Git-F05033?style=for-the-badge&logo=git&logoColor=white)

---

## 📱 Featured Projects

### 🔹 Face Recognition App
- Built using **Flutter + TensorFlow Lite**
- Real-time face detection & recognition  
- Optimized for performance  

### 🔹 Weather App
- Integrated **REST APIs**
- Clean UI + Live weather updates  
- Error handling + loading states  

### 🔹 QR Scanner App
- Built using **Bloc Architecture**
- Fast and efficient scanning  
- Clean UI with state handling  

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=riteshrawat999&show_icons=true&theme=tokyonight&hide_border=true" />
  <img src="https://nirzak-streak-stats.vercel.app/?user=riteshrawat999&theme=tokyonight&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=riteshrawat999&layout=compact&theme=tokyonight&hide_border=true" />
</p>

---

## 🏆 Achievements
- 🚀 Built multiple Flutter apps from scratch  
- 📚 Continuously learning and improving  
- 💡 Strong problem-solving mindset  

---

## 💼 Open to Opportunities
- 👨‍💻 Looking for: **Flutter Developer (Intern / Junior Role)**  
- 🌍 Open to: Remote / On-site  
- 📩 Contact: ritesh.flutter@gmail.com  

---

## 💬 Quote
> "Consistency + Learning + Building = Success 🚀"

---
